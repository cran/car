...
basicPower <- function(U,lambda) {
 bp1 <- function(U,lambda){
  ...
  out <- if(is.matrix(out) | is.data.frame(out)){
    if(is.null(colnames(out))) colnames(out)<- paste("Z",1:dim(out)[2],sep="")
...
estimateTransform <- function(X, Y, weights=NULL,
   ...
   res$start<-start
   res$lambda <- res$par
   names(res$lambda) <- if (is.null(colnames(Y))) paste("Y",1:dim(Y)[2],sep="")
     else colnames(Y)
   roundlam <- res$lambda
...
testTransform.powerTransform <-function(object,lambda=rep(1,dim(object$y)[2])){
   ...
   rownames(out) <- 
     c(paste("LR test, lambda = (",paste(round(lam,2),collapse=" "),")",sep=""))
   out}   
